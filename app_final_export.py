...
st.title("👥 Family or Group Structure Builder")
...
# Diagram title
diagram_title = st.text_input("📛 Title of the Structure", value="Family/Group Structure")
...
rankdir = {"Left to Right": "LR", "Top to Bottom": "TB", "Right to Left": "RL", "Bottom to Top": "BT"}[diagram_direction]
...
if st.button("⬇️ Download PDF Report"):
    ...
    story = [Paragraph(diagram_title, styles["Title"]), Spacer(1, 12)]
...
if st.session_state.entities:
    ...
    dot = Digraph()
    dot.attr(rankdir=rankdir, splines='ortho')
    ...
    clusters = {"Individual": [], "Trust": [], "Company": [], "SMSF": []}
    for ent in st.session_state.entities:
        clusters.get(ent["Type"], []).append(ent)

    for group, members in clusters.items():
        with dot.subgraph(name=f"cluster_{group}") as c:
            c.attr(label=group + "s", style="dashed", color="gray")
            for ent in members:
                ...
                c.node(ent["Entity Name"], label="\n".join(lines), shape=shape, style='filled', color=color, fontcolor="white")

    for rel in st.session_state.relationships:
        dot.edge(rel["From"], rel["To"], xlabel=rel["Relationship"], color="red", style="dashed")
